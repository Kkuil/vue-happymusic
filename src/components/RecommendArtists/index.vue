...
import RecommendArtists from '@/api/recommend_artists.js'
import CoverRow from '@/EncapComponents/CoverRow/index.vue'

const props = defineProps(['title'])
const artists = reactive([])

async function GetRecommendArtists() {
    return await RecommendArtists()
}

onMounted(async () => {
    const data = await GetRecommendArtists()
    console.log(data)
